...
function Product({ data, onPageinfo, val }) {
  return (
    <div>
      <div className="flex max-w-sm flex-col items-center justify-center lg:hover:scale-105 duration-700">
        <div
          onClick={() => onPageinfo(val, data.categories[0].name)}
          className="mt-5 flex flex-col p-5 gap-5 rounded-xl items-center bg-gray-300 shadow-xl"
        >
          <img
            src={data.image.url}
            alt="poster"
